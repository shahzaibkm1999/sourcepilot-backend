 ...
 * ----------------------
 * Generates a Spec-Kit-compliant specification for a project idea and
 * persists it to Supabase. Used by both the HTTP layer and the MCP
 * `create_spec` tool so the two entry points stay in sync (Article II).
 *
 * SourcePilot note: in the broader workflow, spec generation is one
 * stage inside ProjectOrchestrator. SpecificationGenerator remains as a
 * thin, dedicated entry point for the "spec only" flows that already
 * exist (REST `POST /api/specifications/generate` and MCP `create_spec`).
 */
export class SpecificationGenerator {
   ...
   */
  private static readonly SYSTEM_PROMPT = `You are a senior product manager and software architect
...
the plan is the HOW. Maximum three [NEEDS CLARIFICATION] markers total.`;
  ...
  async generateSpecification(projectIdea: string): Promise<GeneratedSpec> {
    ...
    const { content } = await this.deepseek.chat({
      system: SpecificationGenerator.SYSTEM_PROMPT,
      user: `Project idea: ${projectIdea.trim()}`,
      temperature: 0.7,
      maxOutputTokens: 4096,
    });

    return this.parseResponse(content, projectIdea);
  }
  ...
  async createAndSave(projectIdea: string): Promise<{
    ...
    const generated = await this.generateSpecification(projectIdea);
